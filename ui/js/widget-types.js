export const WIDGET_TYPES = {
  stats:   { label:'Stats',         sizes:['small','medium'],        src:(id,item,opts)=>{
    ...
    if (sub === 'disk-health') return `/widgets/disk-health.html?v=23&id=${encodeURIComponent(id)}&size=${size}${opts?.mobile?'&mobile=1':''}`;
    return `/widgets/system-stats.html?v=3&id=${encodeURIComponent(id)}`;
  }},
