export const WIDGET_TYPES = {
  stats:   { label:'Stats',         sizes:['small','medium'],        src:(id,item,opts)=>{
    ...
    if (sub === 'disk-health') return `/widgets/stats/disk-health.html?v=29&id=${encodeURIComponent(id)}&size=${size}${opts?.mobile?'&mobile=1':''}`;
  ...
  adguard: { label:'DNS Server',       sizes:['small','medium'],        src:(id,item)=>`/widgets/adguard/index.html?v=4&id=${encodeURIComponent(id)}&size=${encodeURIComponent(item?.widgetSize||'medium')}` },
