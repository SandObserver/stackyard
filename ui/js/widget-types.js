export const WIDGET_TYPES = {
  stats:   { label:'Stats',         sizes:['small','medium'],        src:(id,item,opts)=>{
    ...
    if (sub === 'disk-health') return `/widgets/stats/disk-health.html?v=24&id=${encodeURIComponent(id)}&size=${size}${opts?.mobile?'&mobile=1':''}`;
  ...
  clock:   { label:'Clock',          sizes:['small'],                 src:(id,item)=>{
    ...
    return `/widgets/clock/${style}.html?v=3&id=${encodeURIComponent(id)}`;
  }},
